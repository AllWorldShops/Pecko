...
class StockMoveLineSerialGenerator(models.TransientModel):
    ...
    first_number = fields.Char(required=False)
...
#             raise ValidationError(_('Only numbers are allowed'))
        if self.stock_move_id.picking_id.partner_id.segment_master_id.code == 'local':
            if self.product_id.categ_id.product_sequence_id:
                seq = self.env['ir.sequence'].browse(self.product_id.categ_id.product_sequence_id.id)
        
        if self.stock_move_id.picking_id.partner_id.segment_master_id.code == 'overseas':
            if self.product_id.categ_id.product_sequence_overseas_id:
                seq = self.env['ir.sequence'].browse(self.product_id.categ_id.product_sequence_overseas_id.id)      
        
        number = int(self.first_number)
        return [seq._next() for i in range(self.qty_to_process)]
