...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        account_ids = self.env['account.account'].search([('user_type_id' , '=' , 2)])
        if request.session['payable_account'] or request.session['receivable_account']:
            if request.session['payable_account'] != False: 
                account = '('+str(request.session['payable_account'])+')'
            elif request.session['receivable_account'] != False: 
                account = '('+str(request.session['receivable_account'])+')'
            else:
                account = tuple(i for i in account_ids.ids)    
        else:
            account = tuple(i for i in account_ids.ids)
        ...
        query = '''
            ...
            WHERE (l.account_id = account_account.id)
                ...
                AND l.account_id IN ''' + str(account) + '''
