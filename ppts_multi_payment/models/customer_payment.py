    ...
    def create_payments(self):
        ...
        if self.credit_line_ids or self.debit_line_ids:
            if self.paid_amount == 0.00:
                for line in self.credit_line_ids:    
                    if line.invoice_id:                
                        move_line_invoice_id = self.env['account.move.line'].search([('invoice_id', '=', line.invoice_id.id), ('reconciled', '=', False), ('debit', '>', 0)])
                    ...
                    for lines in self.debit_line_ids:
                        if line.amount_reconcile != 0.0:
                            if lines.invoice_id:
                                move_line_re_invoice_id = self.env['account.move.line'].search([('invoice_id', '=', lines.invoice_id.id), ('reconciled', '=', False), ('credit', '>', 0)])
                ...
                if self.payment_difference > 0.00:
                    for line in self.credit_line_ids:
                        if line.amount_reconcile >= self.payment_difference:
                            ...
                            break
                    move_line_invoice_id = self.env['account.move.line'].search([('invoice_id', '=', credit_line_id.invoice_id.id), ('reconciled', '=', False), ('debit', '>', 0)])
                    vals={
